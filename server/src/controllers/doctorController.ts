...
export async function submitDoctorAction(req: AuthRequest, res: Response): Promise<void> {
    try {
        ...
        // Emit
        getServerIO().to(`service:${serviceId}`).emit('status_update', { serviceId, status: 'doctor_completed' });
        getServerIO().to(`user:${service.patientId}`).emit('prescription_uploaded', { serviceId });
        if (service.nurseId) getServerIO().to(`user:${service.nurseId}`).emit('status_update', { serviceId, status: 'doctor_completed' });
...
export async function approveProcedure(req: AuthRequest, res: Response): Promise<void> {
    try {
        ...
        getServerIO().to(`service:${serviceId}`).emit('procedure_approved', { serviceId, notes });
        if (service.nurseId) getServerIO().to(`user:${service.nurseId}`).emit('procedure_approved', { serviceId, notes });
...
export async function requestNurseEdit(req: AuthRequest, res: Response): Promise<void> {
    try {
        ...
        getServerIO().to(`service:${serviceId}`).emit('nurse_edit_requested', { serviceId, notes });
        if (service.nurseId) getServerIO().to(`user:${service.nurseId}`).emit('nurse_edit_requested', { serviceId, notes });
