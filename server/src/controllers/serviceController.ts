...
export async function createServiceRequest(req: AuthRequest, res: Response): Promise<void> {
    try {
        ...
        io.to(`user:${patientId}`).emit('slot_confirmed', {
        ...
        if (flowConfig?.isEmergency) {
            ...
            io.to('role:admin').emit('emergency_alert_triggered', {
                serviceId: service.id,
                serviceType,
                patientName: (service as any).patient?.name,
            });
...
export async function updateServiceStatus(req: AuthRequest, res: Response): Promise<void> {
    try {
        ...
        if (newStatus === 'nurse_on_the_way') {
            io.to(`user:${updated.patientId}`).emit('navigation_started', { serviceId: id, nurseName: updated.nurse?.name });
            io.to(`user:${updated.patientId}`).emit('nurse_arrival_estimated', {
                serviceId: id,
                estimatedMinutes: 20,
...
export async function cancelService(req: AuthRequest, res: Response): Promise<void> {
    try {
        ...
        io.to(`user:${updated.patientId}`).emit('status_update', { serviceId: id, status: 'cancelled' });
        if (updated.nurseId) io.to(`user:${updated.nurseId}`).emit('status_update', { serviceId: id, status: 'cancelled' });
        io.to('role:admin').emit('status_update', { serviceId: id, status: 'cancelled' });
