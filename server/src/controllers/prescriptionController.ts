...
export async function handleGeneratePrescription(req: AuthRequest, res: Response): Promise<void> {
    try {
        ...
        // Emit events
        io.to(`service:${serviceId}`).emit('prescription_generated', { serviceId, prescriptionId: prescription.id });
        io.to(`user:${service.patientId}`).emit('prescription_uploaded', { serviceId, pdfUrl: prescription.pdfUrl });
