 ...
 * Routine vaccinations/injections with no adverse reaction can auto-forward.
 */
export function canAutoClose(serviceType: string, vitalsJson: Record<string, any>): boolean {
    ...
    // Routine vaccinations and injections
    const autoCloseServices = [
        'Injection', 'Tetanus (TT) Shot', 'Insulin Injection Help',
        'Hepatitis A', 'Hepatitis B (3 Doses)', 'HPV (Cervical Cancer Vaccine)',
        'Flu Shot', 'Tetanus Booster', 'Rabies Vaccine', 'Rabies Vaccine (Dog Bite)'
    ];

    if (autoCloseServices.includes(serviceType)) {
        return vitalsJson.reactionStatus === 'none';
    }

    // Custom condition from config if any
    if (config.autoCloseCondition) {
        // Simple eval-like check for field matches
        if (config.autoCloseCondition === 'reactionStatus === "none"') {
            return vitalsJson.reactionStatus === 'none';
        }
    }
